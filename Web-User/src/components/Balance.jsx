...
function Balance() {
  ...
  const { chainId, marketAddress, contractABI, tokenAddress, walletAddress } =
    useMoralisDapp();
  ...
  const getMarketItem = (nft) => {
    const result = fetchMarketEvents?.find((e) => e.uri === nft?.token_uri);
    return result;
  };
  ...
  async function updateTicketRemain(nft) {
    ...
    const tickRemain = (
      parseInt(getMarketItem(nft)["ticketRemain"]) + 1
    ).toString();
              ...
              actions={[
                <Tooltip title="View On Blockexplorer">
                  <InfoCircleOutlined
                    onClick={() =>
                      window.open(
                        `${getExplorer(chainId)}token/${nft.token_address}?a=${
                          nft.token_id
                        }`,
                        "_blank"
                      )
