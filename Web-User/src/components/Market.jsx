...
function Market() {
  ...
  const queryMarketEvents = useMoralisQuery("createEvent", (query) =>
    query.equalTo("isClosed", false).notEqualTo("ticketRemain", "0")
  );
  ...
  function failPurchase() {
    ...
    const modal = Modal.error({
      title: "Error!",
      content: `There was a problem when purchasing this ticket`,
    });
    setTimeout(() => {
      modal.destroy();
    }, secondsToGo * 1000);
  ...
  async function updateTicketRemain() {
    const id = Buy["objectId"];
    const tickRemain = (parseInt(Buy["ticketRemain"]) - 1).toString();
  ...
  async function purchase() {
    setLoading(true);
    const tokenDetails = Buy;
    const itemID = tokenDetails["uid"];
    const tokenPrice = tokenDetails["ticketPrice"];
    const ops = {
  ...
  return (
    <>
      <div>
            ...
            <Card
              ...
              actions={[
                <Tooltip title="Read Event Description">
                  <FileSearchOutlined onClick={() => handleClick(index)} />
                </Tooltip>,
                <Tooltip title="Buy Ticket">
                  <ShoppingCartOutlined onClick={() => handleBuyClick(index)} />
                </Tooltip>,
              ]}
        ...
        {Buy ? (
          <Modal
            title={`Ticket price is ${Buy["ticketPrice"] / ("1e" + 18)}  BNB`}
            visible={visible}
